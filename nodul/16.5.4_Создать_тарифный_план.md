---
url: "https://docs.nodul.ru/docs/1ec4d834-f356-80c0-997f-f5ac655cbd72/"
title: "16.5.4 Создать тарифный план | Nodul"
---

**URL:**

```codeBlockLines_e6Vv
https://api.nodul.ru/latenode/v1/whitelabel/plans

```

**Method:**

POST

**Body параметры:**

```codeBlockLines_e6Vv
- Поле:        name
  Тип:         string
  Описание:    Название нового тарифного плана
  ---

- Поле:        features
  Тип:         array
  Описание:    Список характеристик нового тарифного плана
  Содержимое:
    - Поле:     alias
      Тип:      string
      Описание: alias характеристики тарифного плана. Возможные значения:
                min_execution_charging_period_in_mcs — Минимальная плата за исполнение (Микросекунды)
                regular_microcredits — Микрокредиты исполнения
                connected_accounts_limit — Лимит подключенных аккаунтов
                parallel_executions_limit — Лимит параллельных исполнений
                ai_assistant_request_limit — Лимит запросов к AI ассистенту
                plug_and_play_microcredits — Plug&Play Микротокены
                min_triggering_interval_in_seconds — Минимальный интервал триггера (Секунды)
                active_scenarios_limit — Лимит активных сценариев
                exec_history_availability_period_in_min — Период доступности истории исполнения (Минуты)
      ---
    - Поле:     value
      Тип:      object
      Описание: Объект со значением характеристики
      Содержимое:
        - Поле:     int64
          Тип:      string
          Описание: Если значением является число, оно передаётся здесь
          ---
        - Поле:     bool
          Тип:      boolean
          Описание: Если значением является булево значение, оно передаётся здесь

```

**Формат ответа (** `data` **):**

```codeBlockLines_e6Vv
- Поле:        plan
  Тип:         object
  Описание:    Объект с данными созданного тарифного плана
  Содержимое:
    - Поле:     id
      Тип:      string
      Описание: ID нового тарифного плана
      ---
    - Поле:     name
      Тип:      string
      Описание: Название нового тарифного плана
      ---
    - Поле:     status
      Тип:      string
      Описание: Статус плана. Возможные значения:
                plan_status_archived — план в архиве
                plan_status_active — план активен
      ---
    - Поле:     features
      Тип:      array
      Описание: Список характеристик тарифного плана
      Содержимое:
        - Поле:     alias
          Тип:      string
          Описание: alias характеристики тарифного плана
          ---
        - Поле:     value
          Тип:      object
          Описание: Объект со значением характеристики
          Содержимое:
            - Поле: int64
              Тип:  string
              Описание: Числовое значение, если применимо
              ---
            - Поле: bool
              Тип:  boolean
              Описание: Булево значение, если применимо
      ---
    - Поле:     created_at
      Тип:      string
      Описание: Дата создания плана
      ---
    - Поле:     updated_at
      Тип:      string
      Описание: Дата последнего обновления плана

```

**Пример запроса:**

```codeBlockLines_e6Vv
curl --location 'https://api.nodul.ru/latenode/v1/whitelabel/plans?AUTH_TOKEN=YOUR_API_TOKEN'
--header 'Content-Type: application/json'
--data '{
    "name": "Plan Name",
    "features": [\
        {\
            "alias": "active_scenarios_limit",\
            "value": {\
                "int64": "1",\
                "bool": false\
            }\
        },\
        {\
            "alias": "connected_accounts_limit",\
            "value": {\
                "int64": "1",\
                "bool": false\
            }\
        },\
        {\
            "alias": "plug_and_play_microcredits",\
            "value": {\
                "int64": "1000000",\
                "bool": false\
            }\
        },\
        {\
            "alias": "min_triggering_interval_in_seconds",\
            "value": {\
                "int64": "120",\
                "bool": false\
            }\
        },\
        {\
            "alias": "min_execution_charging_period_in_mcs",\
            "value": {\
                "int64": "3000000",\
                "bool": false\
            }\
        },\
        {\
            "alias": "exec_history_availability_period_in_min",\
            "value": {\
                "int64": "1",\
                "bool": false\
            }\
        },\
        {\
            "alias": "regular_microcredits",\
            "value": {\
                "int64": "1000000",\
                "bool": false\
            }\
        },\
        {\
            "alias": "parallel_executions_limit",\
            "value": {\
                "int64": "1",\
                "bool": false\
            }\
        },\
        {\
            "alias": "ai_assistant_request_limit",\
            "value": {\
                "int64": "1",\
                "bool": false\
            }\
        }\
    ]
}'

```

**Пример ответа:**

```codeBlockLines_e6Vv
{
    "success": true,
    "request_id": "Iit8HDuiyKSO6CuGGHzW",
    "data": {
        "plan": {
            "id": "0",
            "name": "Plan Name",
            "status": "plan_status_active",
            "features": [\
                {\
                    "alias": "plug_and_play_microcredits",\
                    "value": {\
                        "int64": "1000000",\
                        "bool": false\
                    }\
                },\
                {\
                    "alias": "min_execution_charging_period_in_mcs",\
                    "value": {\
                        "int64": "3000000",\
                        "bool": false\
                    }\
                },\
                {\
                    "alias": "parallel_executions_limit",\
                    "value": {\
                        "int64": "1",\
                        "bool": false\
                    }\
                },\
                {\
                    "alias": "ai_assistant_request_limit",\
                    "value": {\
                        "int64": "1",\
                        "bool": false\
                    }\
                },\
                {\
                    "alias": "active_scenarios_limit",\
                    "value": {\
                        "int64": "1",\
                        "bool": false\
                    }\
                },\
                {\
                    "alias": "connected_accounts_limit",\
                    "value": {\
                        "int64": "1",\
                        "bool": false\
                    }\
                },\
                {\
                    "alias": "min_triggering_interval_in_seconds",\
                    "value": {\
                        "int64": "120",\
                        "bool": false\
                    }\
                },\
                {\
                    "alias": "exec_history_availability_period_in_min",\
                    "value": {\
                        "int64": "1",\
                        "bool": false\
                    }\
                },\
                {\
                    "alias": "regular_microcredits",\
                    "value": {\
                        "int64": "1000000",\
                        "bool": false\
                    }\
                }\
            ],
            "created_at": "2025-05-05T14:57:47.716Z",
            "updated_at": "2025-05-05T14:57:47.716Z"
        }
    },
    "errors": []
}

```